...
impl Plugin for MainMenuPlugin {
    fn build(&self, app: &mut AppBuilder) {
        ...
        .add_system_set(
            SystemSet::on_resume(GameState::MainMenu).with_system(main_menu_setup.system()),
        )
        .add_system_set(
...
fn despawn_title(mut query: Query<Entity, With<MainScreenRelated>>, mut commands: Commands) {
    for entity in query.iter_mut() {
        commands.entity(entity).despawn();
    }
}

fn despawn_start_button(mut query: Query<Entity, With<StartBtn>>, mut commands: Commands) {
    for entity in query.iter_mut() {
        commands.entity(entity).despawn();
    }
}
